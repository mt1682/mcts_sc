...
#define CHILD_MAX  (B_SIZE*B_SIZE+1)  // +1 for PASS
...
int select_best_ucb(int node_n)
{
  ...
  for (i=0; i<pN->child_num; i++) {
    ...
    if ( c->games==0 ) {
      ...
      const double C = 0.31; // depends on program
...
int main()
{
	...
	for (;;) {
		...
		z = select_best_move(color); // pure monte-calro
#else
		z = select_best_uct(color);  // UCT
#endif
		...
		t = (double)(clock()+1 - bt) / CLOCKS_PER_SEC;
